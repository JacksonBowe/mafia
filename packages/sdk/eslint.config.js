...
//   @mafia/core/game/session/chat
const CORE_IMPORT_RESTRICTION = {
	regex:
		'^@mafia/core(?!/(message|error/schema|db/schema|lobby/schema|user/schema|game/schema|game/session/chat)$)(/.*)?$',
	message:
		'SDK may only import from pure @mafia/core schema paths (message, error/schema, db/schema, lobby/schema, user/schema, game/schema, game/session/chat).',
};
